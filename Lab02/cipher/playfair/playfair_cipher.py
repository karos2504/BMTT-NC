class PlayFairCipher:
        ...
        for row in range(len(matrix)):
            for col in range(len(matrix[row])):
                if matrix[row][col] == letter:
                    return row, col
        return None  # Trả về None nếu không tìm thấy (phòng trường hợp lỗi)

    def playfair_encrypt(self, plain_text, matrix):
        plain_text = plain_text.upper().replace("J", "I")  # Chuẩn hóa văn bản
        encrypted_text = ""
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        # Loại bỏ ký tự 'X' nếu nó là ký tự được thêm vào
